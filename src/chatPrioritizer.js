...
exports.prioritizeChatMessages = prioritizeChatMessages;
exports.prioritizeWithConversationFlow = prioritizeWithConversationFlow;
const embeddings_1 = require("./embeddings");
async function prioritizeChatMessages(messages, queryText, strategy = "hybrid", embeddingProvider) {
    ...
    const messageMetadata = messages.map((msg, index) => ({
        index,
        text: msg.content,
        role: msg.role,
        originalIndex: index,
...
function prioritizeByRecency(messages) {
    return messages
        .sort((a, b) => (b.timestamp?.getTime() || 0) - (a.timestamp?.getTime() || 0));
}
function prioritizeBySemanticRelevance(messages, queryEmbedding) {
    const scored = messages.map(msg => {
        if (!msg.embedding) {
            return { ...msg, relevanceScore: 0 };
        }
        const similarity = (0, embeddings_1.cosineSimilarity)(queryEmbedding, msg.embedding);
        return { ...msg, relevanceScore: similarity };
    });
    return scored
        .sort((a, b) => (b.relevanceScore || 0) - (a.relevanceScore || 0));
}
function prioritizeSemanticHybridForChat(messages, queryEmbedding) {
    const now = Date.now();
    const scored = messages.map(msg => {
        // Semantic relevance score
        let relevanceScore = 0;
        if (msg.embedding) {
            relevanceScore = (0, embeddings_1.cosineSimilarity)(queryEmbedding, msg.embedding);
        }
        ...
        const hybridScore = (relevanceScore * 0.4 + // 40% relevance
...
function getRoleImportanceScore(role) {
    switch (role) {
        case "system":
...
async function prioritizeWithConversationFlow(messages, queryText, embeddingProvider, maxMessages = 10) {
    ...
    const prioritizedRemaining = await prioritizeChatMessages(remainingMessages, queryText, "hybrid", embeddingProvider);
...
//# sourceMappingURL=chatPrioritizer.js.map